...
class DataObjects_Tag extends DB_DataObject_Pluggable
{
    ...
    public function tagged_at_for($record)
    {
      $db = $this->getDatabaseConnection();
      return $db->queryOne('select tagged_at from tag_record where tag_id = '.$this->id.' and record_id='.$db->quote($record->pk()).' and tagged_table="'.$record->tableName().'"');
    }
