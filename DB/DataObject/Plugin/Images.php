...
class DB_DataObject_Plugin_Images extends DB_DataObject_Plugin
{
    ...
    HTML_QuickForm::registerElementType('imagefile','M/HTML/QuickForm/imagefile.php','HTML_QuickForm_imagefile');
		foreach($obj->photoFields as $k=>$v){
				...
				$newtaille=$v['x']&&$v['y']?$v['x'].' par '.$v['y'].' pixels':
				($v['x']&&!$v['y']?$v['x'].' pixels de large (proportions conservées)':
					(!$v['x']&&$v['y']?$v['y'].' pixels de haut (proportions conservées)':''));
			$obj->fb_fieldLabels[$k] = array($obj->fb_fieldLabels[$k],'note'=>$newtaille?". L'image sera redimensionnée en ".$newtaille:'');
            if(!empty($obj->$k)) {
                //TODO move this to postGenerateForm (like in upload plugin)
                $obj->fb_fieldLabels[$k]['unit']='<input type="checkbox" name="__image_delete_'.$k.'" value="1" />'.__('Supprimer l\'image');
            }
